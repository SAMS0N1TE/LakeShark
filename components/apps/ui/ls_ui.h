...
/* `controls` is the row itself.  A value row is one wrapping line -
...
/* primitives still let every app invent its own chrome.  This is the
...
typedef struct {
    ...
    lv_obj_t *readout;
    lv_obj_t *lamp;
    /* the kit-owned strip of tab buttons.  `tabs` is still the
    ...
    lv_obj_t *tabbar;
    lv_obj_t *pager; /* Compact previous/caption/next row; caption reveals direct tabs. */
    lv_obj_t *tabs;
    lv_obj_t *content;
    lv_obj_t *controls;
} ls_ui_screen_t;

/* a tab whose action row is never scrolled away.  The body takes
...
const char *ls_ui_page_title(void);
void ls_ui_clear_page_title(void);
 ...
 * number of choices the way a button per choice does (). */
...
/* A readout placed directly on a column panel, not inside a value
...
lv_obj_t *ls_ui_hold_button(lv_obj_t *parent, const char *text,
                            ...
                            lv_event_cb_t callback, void *user_data);
/* one full-width line stating the hold rule before anything is
