    ...
    while (i < n && y + bh <= area.y + area.h) {
        ...
        for (int placed = 0; placed < here && i < n; placed++, i++) {
            ...
            if (two_way(&items[i])) {
                ...
                if(wide && items[i].key_down){char badge[]={'[',items[i].key_down,']',0};tui_put_str(sf,area,x+2,y+bh-1,badge,have?lab:dim);}
                if(wide && items[i].key){char badge[]={'[',items[i].key,']',0};tui_put_str(sf,area,x+half+2,y+bh-1,badge,have?lab:dim);}
                continue;
            }
            ...
            if(wide && items[i].key){char badge[]={'[',items[i].key,']',0};tui_put_str(sf,area,x+w-5,y+bh-1,badge,have?lab:dim);}
        }
