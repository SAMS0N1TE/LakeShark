                           ...
                           char *serial)
{
    ...
    {
        const char *sp = (dev_info.speed == USB_SPEED_HIGH) ? "HIGH(480M)"
                       : (dev_info.speed == USB_SPEED_FULL) ? "FULL(12M)"
                       : (dev_info.speed == USB_SPEED_LOW)  ? "LOW(1.5M)" : "?";
        ESP_LOGW("RTLSDR", "*** USB negotiated speed: %s (enum=%d) -- need HIGH for 960kSPS ***",
                 sp, (int)dev_info.speed);
    }
...
/* Streaming API: post a self-resubmitting transfer pool feeding a PSRAM IQ ring
