...
{
    if (valid_nids == 0 && valid_tsbks == 0) return P25_QUAL_NO_SCORE;
    uint64_t score = (uint64_t)valid_nids + (uint64_t)valid_tsbks * 4u;
    return score > INT_MAX ? INT_MAX : (int)score;
}
