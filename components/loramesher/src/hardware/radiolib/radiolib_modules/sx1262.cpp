...
Result LoraMesherSX1262::Begin(const RadioConfig& config) {
    ...
    // Route the TX/RX antenna switch to DIO2 so the chip automatically
    // toggles the RF path between transmit and receive. Required by SX1262
    // modules that wire their antenna switch to DIO2 (e.g. Seeed Wio-SX1262).
    status = radio_module_->setDio2AsRfSwitch(true);
    if (status != RADIOLIB_ERR_NONE) {
        return RadioLibCodeErrors::ConvertStatus(status);
    }
