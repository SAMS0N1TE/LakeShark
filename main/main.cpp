...
#include "esp_log.h"
#include "ls_sdcard.h"
/* ls_board.h, not ls_caps.h. The capabilities header refuses to
   be included on its own - it derives LS_HAS_* from the variant and has
   an #error saying so - and it was reached directly from here. It built
   on four boards because something else pulled ls_board.h in first and
   the guard passed; on p4-touch-lcd-43 and -4b the include order differs
   and the build stopped. Including the header that owns it works
   everywhere and is what the error asks for. */
#include "ls_board.h"
#include "esp_err.h"
#include "esp_check.h"
...
/* Pull-down shade actions. The shade UI lives under components/apps/
...
{
    /* Presence probe only. /initialization must never enter the
    ...
    /* create the independent, higher-priority CLI task before any
...
extern "C" void app_main(void)
{
    ...
    /* prepare the statically reserved cache-safe NVS dispatcher before
...
#elif CONFIG_EXAMPLE_ENABLE_SD_CARD
    /**/
    {
    ...
    /* Wire the shade to the GUI-build symbols before the first app
    ...
    /* Reaching here is not yet "healthy" - most of what crashes this
